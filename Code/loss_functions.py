...
def lp_loss(gen_frames, gt_frames, l_num):
    ...
    gen_fr = np.array(gen_frames)
    gt_fr = np.array(gt_frames)
    
    '''
    gen_fr = np.add(np.array(gen_frames), 1)
    gt_fr = np.add(np.array(gt_frames), 1)
    
    gen_sum = np.add(gen_fr[..., 1], gen_fr[..., 2])
    gt_sum = np.add(gt_fr[..., 1], gt_fr[..., 2])

    gen_max = np.maximum(gen_sum, 1)
    gt_max = np.maximum(gt_sum, 1)

    gen_div = np.divide(gen_fr[..., 0], gen_max)
    gt_div = np.divide(gt_fr[..., 0], gt_max)

    more_red = 2

    gen_more_red = more_red * gen_div 
    gt_more_red = more_red * gt_div
    
    gen_fr[..., 0] *= gen_more_red
    gt_fr[..., 0] *= gt_more_red
    '''
    ...
    for i in range(len(gen_fr)):
        scale_losses.append(tf.reduce_sum(tf.abs(gen_fr[i] - gt_fr[i])**l_num))
