...
class GeneratorModel:
    ...
    def define_graph(self):
        ...
        with tf.name_scope('generator'):
            ...
            with tf.name_scope('train'):
                ...
                # train loss summary
                loss_summary = tf.summary.scalar('train_loss_G', self.global_loss) #COMMENTED
                self.summaries_train.append(loss_summary) #COMMENTED
    ...
    def train_step(self, batch, discriminator=None):
        ...
        if global_step % c.SUMMARY_FREQ == -1: # CHANGED!!!!
        ...
        if global_step % c.IMG_SAVE_FREQ == 0:
            ...
            for pred_num in range(len(input_frames)):
                ...
                for frame_num in range(c.HIST_LEN):
                    ...
                    im.save(os.path.join(pred_dir, 'input_' + str(frame_num) + '.png'), "PNG")
                ...
                for scale_num, scale_pred in enumerate(scale_preds):
                    ...
                    gen_img = (gen_img + 1) * (255 / 2)

                    path = os.path.join(pred_dir, 'scale' + str(scale_num))
                    gt_img = scale_gts[scale_num][pred_num]
                    gt_img = (gt_img + 1) * (255 / 2)

                    im = Image.fromarray(gen_img.astype('uint8'), 'RGB')
                    im.save(path + '_gen.png', "PNG")

                    im = Image.fromarray(gt_img.astype('uint8'), 'RGB')
                    im.save(path + '_gt.png', "PNG")
    ...
    def test_batch(self, batch, global_step, num_rec_out=1, save_imgs=True):
        ...
        #self.summary_writer.add_summary(rec_summaries[0], global_step) CHANGED!!!!!!!!!!
        ...
        if save_imgs:
            for pred_num in range(len(input_frames)):
                pred_dir = c.get_dir(c.IMG_SAVE_DIR)
                ...
                """for frame_num in range(c.HIST_LEN):
                    ...
                    img = (img + 1) * (255 / 2)
                    im = Image.fromarray(img.astype('uint8'), 'RGB')
                    im.save(os.path.join(pred_dir, 'input_' + str(frame_num) + '.jpg'), "JPEG")
                    #imsave(os.path.join(pred_dir, 'input_' + str(frame_num) + '.'), img)"""
                ...
                for rec_num in range(num_rec_out):
                    ...
                    #gt_img = (gt_img + 1) * (255 / 2)
                    im = Image.fromarray(gen_img.astype('uint8'), 'RGB')
                    im.save(os.path.join(pred_dir, 'gen_' + str(rec_num) + '.jpg'), "JPEG")
                    #im = Image.fromarray(gt_img.astype('uint8'), 'RGB')
                    #im.save(os.path.join(pred_dir, 'gt_' + str(rec_num) + '.jpg'), "JPEG")
